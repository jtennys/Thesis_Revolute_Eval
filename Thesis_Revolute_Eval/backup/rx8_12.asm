...
;  Global Symbols
;-----------------------------------------------
export   RX8_12_EnableInt
export  _RX8_12_EnableInt
export   RX8_12_DisableInt
export  _RX8_12_DisableInt
export   RX8_12_Start
export  _RX8_12_Start
export   RX8_12_Stop
export  _RX8_12_Stop
export   RX8_12_bReadRxData
export  _RX8_12_bReadRxData
export   RX8_12_bReadRxStatus
export  _RX8_12_bReadRxStatus

; Old function name convension, do not use.
; These will be removed in a future release.
export  bRX8_12_ReadRxData
export _bRX8_12_ReadRxData
export  bRX8_12_ReadRxStatus
export _bRX8_12_ReadRxStatus

;-----------------------------------------------
; High Level RX functions
;-----------------------------------------------

export  RX8_12_cGetChar
export _RX8_12_cGetChar
export  RX8_12_cReadChar
export _RX8_12_cReadChar
export  RX8_12_iReadChar
export _RX8_12_iReadChar

IF (RX8_12_RXBUF_ENABLE)
export  RX8_12_CmdReset
export _RX8_12_CmdReset
export  RX8_12_bCmdCheck
export _RX8_12_bCmdCheck
export  RX8_12_bCmdLength
export _RX8_12_bCmdLength
export  RX8_12_bErrCheck
export _RX8_12_bErrCheck

export  RX8_12_szGetParam
export _RX8_12_szGetParam
export  RX8_12_szGetRestOfParams
export _RX8_12_szGetRestOfParams

;-----------------------------------------------
;  Variables
;-----------------------------------------------

AREA RX8_12_RAM(RAM,REL,CON)
 ptrParam:   			BLK  1
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_EnableInt
;
;  DESCRIPTION:
...
_RX8_12_EnableInt:
   ...
   M8C_EnableIntMask RX8_12_INT_REG, RX8_12_bINT_MASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_DisableInt
;
;  DESCRIPTION:
...
_RX8_12_DisableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   M8C_DisableIntMask RX8_12_INT_REG, RX8_12_bINT_MASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_Start(BYTE bParity)
;
;  DESCRIPTION:
...
_RX8_12_Stop:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   REG[RX8_12_CONTROL_REG], ~bfCONTROL_REG_START_BIT
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_bReadRxData
;
;  DESCRIPTION:
...
_bRX8_12_ReadRxData:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A, REG[RX8_12_RX_BUFFER_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_bReadRxStatus
;
;  DESCRIPTION:
...
;    functions.
;
 RX8_12_bReadRxStatus:
_RX8_12_bReadRxStatus:
 bRX8_12_ReadRxStatus:
_bRX8_12_ReadRxStatus:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A,  REG[RX8_12_CONTROL_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.getChar_Loop:
   tst REG[RX8_12_CONTROL_REG],RX8_12_RX_REG_FULL   ; Check if a character is ready
   jz  .getChar_Loop                                        ; If not loop

   mov A, REG[RX8_12_RX_BUFFER_REG]              ; Get character
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_cReadChar
;
;  DESCRIPTION:
;     Read character from UART RX port.
;
...
_RX8_12_cReadChar:
   ...
   mov  A,REG[RX8_12_CONTROL_REG]                          ; Get Status of RX
   push A
   and  A,RX8_12_RX_COMPLETE                               ; Check if a character is ready
   jnz  .RX_DATA_RDY                                       ; Data Ready go read it.
   pop  A
   jmp  .RX_NO_VALID_CHAR

.RX_DATA_RDY:
   mov  A,REG[RX8_12_RX_BUFFER_REG]                        ; Read data first, then
   swap A,X                                                ; determine if data is valid

   pop  A                                                  ; Check for errors
   and  A,(RX8_12_RX_PARITY_ERROR | RX8_12_RX_FRAMING_ERROR)
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_iReadChar
;
...
_RX8_12_iReadChar:
   ...
   and  A,(RX8_12_RX_ERROR|RX8_12_RX_REG_FULL)
   push A
   and  A,RX8_12_RX_COMPLETE                               ; Check if a character is ready
   jnz  .RX_GET_DATA                                       ; Data Ready go read it.
   pop  A
   or   A,RX8_12_RX_NO_DATA                                ; Add no data flag
   swap A,X
   jmp  End_RX8_12_iReadChar

.RX_GET_DATA:
   pop  A
   and  A,RX8_12_RX_ERROR
   swap A,X
   mov  A,REG[RX8_12_RX_BUFFER_REG]                        ; Read data first, then
                                                           ; determine if data is valid

 End_RX8_12_iReadChar:
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
_RX8_12_CmdReset:
   ...
   RAM_SETPAGE_CUR >RX8_12_aRxBuffer
   mov [RX8_12_aRxBuffer], 0x00
   RAM_SETPAGE_CUR >ptrParam
   mov [ptrParam],0x00
   RAM_SETPAGE_CUR >RX8_12_bRxCnt
   mov [RX8_12_bRxCnt], 0x00
   and [RX8_12_fStatus], 0x00
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_bCmdCheck
;
;  DESCRIPTION:
...
_RX8_12_bCmdCheck:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >RX8_12_fStatus
   mov A,  [RX8_12_fStatus]
   and A, RX8_12_RX_BUF_CMDTERM                  ; Mask off Command status
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_bErrCheck
;
;  DESCRIPTION:
...
_RX8_12_bErrCheck:
   ...
   RAM_SETPAGE_CUR >RX8_12_fStatus
   mov A,  [RX8_12_fStatus]
   and A, RX8_12_RX_BUF_ERROR                    ; Mask off Error status
   and [RX8_12_fStatus], ~RX8_12_RX_BUF_ERROR
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_bCmdLength
;
;  DESCRIPTION:
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_szGetParam
;
;  DESCRIPTION:
...
;     RX8_12_CmdReset which resets the entire buffer to the 'empty'
;     state. Typically this function, RX8_12_szGetParam, is
;     not called until the buffer has been loaded with an entire command
;     (See RX8_12_bCmdCheck).
;
...
_RX8_12_szGetParam:
   ...
   RAM_SETPAGE_CUR >ptrParam
   RAM_SETPAGE_IDX >RX8_12_aRxBuffer

   mov  A, <RX8_12_aRxBuffer                ; Get address to receive buffer
...
.CheckForDelim:
    cmp  A,RX8_12_DELIMITER                 ; Check if we have a delimiter
    jnz  .ParamStartFound
    inc  X                                  ; Increment both current pointer and
    inc  [ptrParam]                         ; stored pointer.
    mov  A,[X]                              ; Get character pointed by X
    cmp  [ptrParam],(RX8_12_RX_BUFFER_SIZE -1)  ; Check if we are at the end of buffer
    jnz  .CheckForDelim
                                            ; End of string found
.EndOfString:
    push X                                  ; Save ptr
...
.ParamLoop:
    ...
    mov  [ptrParam],(RX8_12_RX_BUFFER_SIZE -1)  ; Reset pointer to end of string.
...
.NotNullString:
   pop  X
   mov  A,>RX8_12_aRxBuffer                     ; Return pointer
   RAM_EPILOGUE RAM_USE_CLASS_3
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RX8_12_szGetRestOfParams
;
;  DESCRIPTION:
...
;          CUR_PP
;
 RX8_12_szGetRestOfParams:
_RX8_12_szGetRestOfParams:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >ptrParam

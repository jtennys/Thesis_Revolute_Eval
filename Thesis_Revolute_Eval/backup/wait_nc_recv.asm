...
include "m8c.inc"
include "memory.inc"
include "WAIT_NC_RECV.inc"


;-----------------------------------------------
;  Global Symbols
;-----------------------------------------------
export   WAIT_NC_RECV_EnableInt
export  _WAIT_NC_RECV_EnableInt
export   WAIT_NC_RECV_DisableInt
export  _WAIT_NC_RECV_DisableInt
export   WAIT_NC_RECV_Start
export  _WAIT_NC_RECV_Start
export   WAIT_NC_RECV_Stop
export  _WAIT_NC_RECV_Stop
export   WAIT_NC_RECV_bReadRxData
export  _WAIT_NC_RECV_bReadRxData
export   WAIT_NC_RECV_bReadRxStatus
export  _WAIT_NC_RECV_bReadRxStatus

; Old function name convension, do not use.
; These will be removed in a future release.
export  bWAIT_NC_RECV_ReadRxData
export _bWAIT_NC_RECV_ReadRxData
export  bWAIT_NC_RECV_ReadRxStatus
export _bWAIT_NC_RECV_ReadRxStatus

;-----------------------------------------------
; High Level RX functions
;-----------------------------------------------

export  WAIT_NC_RECV_cGetChar
export _WAIT_NC_RECV_cGetChar
export  WAIT_NC_RECV_cReadChar
export _WAIT_NC_RECV_cReadChar
export  WAIT_NC_RECV_iReadChar
export _WAIT_NC_RECV_iReadChar

IF (WAIT_NC_RECV_RXBUF_ENABLE)
export  WAIT_NC_RECV_CmdReset
export _WAIT_NC_RECV_CmdReset
export  WAIT_NC_RECV_bCmdCheck
export _WAIT_NC_RECV_bCmdCheck
export  WAIT_NC_RECV_bCmdLength
export _WAIT_NC_RECV_bCmdLength
export  WAIT_NC_RECV_bErrCheck
export _WAIT_NC_RECV_bErrCheck

export  WAIT_NC_RECV_szGetParam
export _WAIT_NC_RECV_szGetParam
export  WAIT_NC_RECV_szGetRestOfParams
export _WAIT_NC_RECV_szGetRestOfParams

;-----------------------------------------------
;  Variables
;-----------------------------------------------

AREA WAIT_NC_RECV_RAM(RAM,REL,CON)
 ptrParam:   			BLK  1
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_EnableInt
;
;  DESCRIPTION:
...
;    functions.
;
 WAIT_NC_RECV_EnableInt:
_WAIT_NC_RECV_EnableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   M8C_EnableIntMask WAIT_NC_RECV_INT_REG, WAIT_NC_RECV_bINT_MASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_DisableInt
;
;  DESCRIPTION:
...
;    functions.
;
 WAIT_NC_RECV_DisableInt:
_WAIT_NC_RECV_DisableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   M8C_DisableIntMask WAIT_NC_RECV_INT_REG, WAIT_NC_RECV_bINT_MASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_Start(BYTE bParity)
;
;  DESCRIPTION:
...
_WAIT_NC_RECV_Start:
   ...
   mov   REG[WAIT_NC_RECV_CONTROL_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_Stop
;
;  DESCRIPTION:
...
_WAIT_NC_RECV_Stop:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   REG[WAIT_NC_RECV_CONTROL_REG], ~bfCONTROL_REG_START_BIT
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_bReadRxData
;
;  DESCRIPTION:
...
;    functions.
;
 WAIT_NC_RECV_bReadRxData:
_WAIT_NC_RECV_bReadRxData:
 bWAIT_NC_RECV_ReadRxData:
_bWAIT_NC_RECV_ReadRxData:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A, REG[WAIT_NC_RECV_RX_BUFFER_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_bReadRxStatus
;
;  DESCRIPTION:
...
;    functions.
;
 WAIT_NC_RECV_bReadRxStatus:
_WAIT_NC_RECV_bReadRxStatus:
 bWAIT_NC_RECV_ReadRxStatus:
_bWAIT_NC_RECV_ReadRxStatus:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A,  REG[WAIT_NC_RECV_CONTROL_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_cGetChar
;
;  DESCRIPTION:
...
;
 WAIT_NC_RECV_cGetChar:
_WAIT_NC_RECV_cGetChar:
   RAM_PROLOGUE RAM_USE_CLASS_1

.getChar_Loop:
   tst REG[WAIT_NC_RECV_CONTROL_REG],WAIT_NC_RECV_RX_REG_FULL   ; Check if a character is ready
   jz  .getChar_Loop                                        ; If not loop

   mov A, REG[WAIT_NC_RECV_RX_BUFFER_REG]        ; Get character
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_cReadChar
;
;  DESCRIPTION:
;     Read character from UART RX port.
;
...
;
 WAIT_NC_RECV_cReadChar:
_WAIT_NC_RECV_cReadChar:
   RAM_PROLOGUE RAM_USE_CLASS_1

   mov  A,REG[WAIT_NC_RECV_CONTROL_REG]                    ; Get Status of RX
   push A
   and  A,WAIT_NC_RECV_RX_COMPLETE                         ; Check if a character is ready
   jnz  .RX_DATA_RDY                                       ; Data Ready go read it.
   pop  A
   jmp  .RX_NO_VALID_CHAR

.RX_DATA_RDY:
   mov  A,REG[WAIT_NC_RECV_RX_BUFFER_REG]                  ; Read data first, then
   swap A,X                                                ; determine if data is valid

   pop  A                                                  ; Check for errors
   and  A,(WAIT_NC_RECV_RX_PARITY_ERROR | WAIT_NC_RECV_RX_FRAMING_ERROR)
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_iReadChar
;
...
;    functions.
;
 WAIT_NC_RECV_iReadChar:
_WAIT_NC_RECV_iReadChar:
   RAM_PROLOGUE RAM_USE_CLASS_1

   mov  A,REG[WAIT_NC_RECV_CONTROL_REG]                    ; Get Status of RX
                                                           ; Mask only errors and data ready
   and  A,(WAIT_NC_RECV_RX_ERROR|WAIT_NC_RECV_RX_REG_FULL)
   push A
   and  A,WAIT_NC_RECV_RX_COMPLETE                         ; Check if a character is ready
   jnz  .RX_GET_DATA                                       ; Data Ready go read it.
   pop  A
   or   A,WAIT_NC_RECV_RX_NO_DATA                          ; Add no data flag
   swap A,X
   jmp  End_WAIT_NC_RECV_iReadChar

.RX_GET_DATA:
   pop  A
   and  A,WAIT_NC_RECV_RX_ERROR
   swap A,X
   mov  A,REG[WAIT_NC_RECV_RX_BUFFER_REG]                  ; Read data first, then
                                                           ; determine if data is valid

 End_WAIT_NC_RECV_iReadChar:
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_CmdReset
;
;  DESCRIPTION:
...
;
 WAIT_NC_RECV_CmdReset:
_WAIT_NC_RECV_CmdReset:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >WAIT_NC_RECV_aRxBuffer
   mov [WAIT_NC_RECV_aRxBuffer], 0x00
   RAM_SETPAGE_CUR >ptrParam
   mov [ptrParam],0x00
   RAM_SETPAGE_CUR >WAIT_NC_RECV_bRxCnt
   mov [WAIT_NC_RECV_bRxCnt], 0x00
   and [WAIT_NC_RECV_fStatus], 0x00
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_bCmdCheck
;
;  DESCRIPTION:
...
;
 WAIT_NC_RECV_bCmdCheck:
_WAIT_NC_RECV_bCmdCheck:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >WAIT_NC_RECV_fStatus
   mov A,  [WAIT_NC_RECV_fStatus]
   and A, WAIT_NC_RECV_RX_BUF_CMDTERM            ; Mask off Command status
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_bErrCheck
;
;  DESCRIPTION:
...
;
 WAIT_NC_RECV_bErrCheck:
_WAIT_NC_RECV_bErrCheck:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >WAIT_NC_RECV_fStatus
   mov A,  [WAIT_NC_RECV_fStatus]
   and A, WAIT_NC_RECV_RX_BUF_ERROR              ; Mask off Error status
   and [WAIT_NC_RECV_fStatus], ~WAIT_NC_RECV_RX_BUF_ERROR
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_bCmdLength
;
;  DESCRIPTION:
...
;          CUR_PP
;
 WAIT_NC_RECV_bCmdLength:
_WAIT_NC_RECV_bCmdLength:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >WAIT_NC_RECV_bRxCnt
   mov A,  [WAIT_NC_RECV_bRxCnt]
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_szGetParam
;
;  DESCRIPTION:
...
;     WAIT_NC_RECV_CmdReset which resets the entire buffer to the 'empty'
;     state. Typically this function, WAIT_NC_RECV_szGetParam, is
;     not called until the buffer has been loaded with an entire command
;     (See WAIT_NC_RECV_bCmdCheck).
;
...
;
 WAIT_NC_RECV_szGetParam:
_WAIT_NC_RECV_szGetParam:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_PROLOGUE RAM_USE_CLASS_3
   RAM_SETPAGE_CUR >ptrParam
   RAM_SETPAGE_IDX >WAIT_NC_RECV_aRxBuffer

   mov  A, <WAIT_NC_RECV_aRxBuffer          ; Get address to receive buffer
...
.CheckForDelim:
    cmp  A,WAIT_NC_RECV_DELIMITER           ; Check if we have a delimiter
    jnz  .ParamStartFound
    inc  X                                  ; Increment both current pointer and
    inc  [ptrParam]                         ; stored pointer.
    mov  A,[X]                              ; Get character pointed by X
    cmp  [ptrParam],(WAIT_NC_RECV_RX_BUFFER_SIZE -1)  ; Check if we are at the end of buffer
    jnz  .CheckForDelim
                                            ; End of string found
.EndOfString:
    push X                                  ; Save ptr
...
.ParamLoop:
    ...
    mov  [ptrParam],(WAIT_NC_RECV_RX_BUFFER_SIZE -1)  ; Reset pointer to end of string.
...
.NotNullString:
   pop  X
   mov  A,>WAIT_NC_RECV_aRxBuffer                     ; Return pointer
   RAM_EPILOGUE RAM_USE_CLASS_3
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: WAIT_NC_RECV_szGetRestOfParams
;
;  DESCRIPTION:
...
;          CUR_PP
;
 WAIT_NC_RECV_szGetRestOfParams:
_WAIT_NC_RECV_szGetRestOfParams:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >ptrParam
